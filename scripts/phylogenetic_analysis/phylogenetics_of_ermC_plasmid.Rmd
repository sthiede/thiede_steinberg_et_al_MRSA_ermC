...
# 
# Methodology: Ancestral state reconstruction using phyloAMR (https://github.com/kylegontjes/phyloAMR)
## 1. Identified ancestral states using phyloAMR's asr() function 
## 2. Identified phylogenetic clusters of ermC plasmids using phyloAMR's asr_cluster_detection() function
## 3. Visualization using ggtree and ggplot2 functions
# 
...
# Manipulate phylogeny
```{r}
# Midpoint phylogeny and match ordering
...
```{r}
transition_stats <- phyloAMR::asr_transition_analysis(ermC_asr$parent_child_df)
...
```{r}
ermC_asr_cluster <- asr_cluster_detection(df = df, tr = tr_midpoint, tip_name_variable = "seq_id",patient_id = 'patient_id',parent_child_df = ermC_asr$parent_child_df, node_states = 'joint', simplify_faux_clusters = TRUE, simplify_revertant = TRUE, collapse_cluster =  TRUE)
...
clusters_col <- hues::iwanthue(ncluster,hmin=13,hmax=360,lmin = 0,lmax = 100,cmin=0,cmax=180,random = F,plot = T)
...
## Histogram
```{r, fig.height=7,fig.width=7.5}
B <- ggplot(data = df, aes(x = date_bin, fill = fct_rev(fct_infreq(asr_cluster_renamed_rev)))) + geom_histogram(stat  = 'count') + format + theme_bw_me + theme(legend.position='none')  + xlab("Time (months)") + ylab("Number of isolates") +  cluster_scale 
...
# Supplemental figure on phylogenetics of ermC in the dataset
